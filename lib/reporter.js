...
const isArray = R.allPass([R.isArrayLike, R.pipe(R.length, R.gt(R.__, 0))]);
const isError = (code) => code && code[0] === 'E';
const createWriter =  R.curry(
  (testing, msg) => testing === true ? msg : process.stdout.write(msg)
);
...
function prepareContent(results) {
  return results.reduce((obj, element) => {
    if (obj.previous !== element.file) {
      ...
      obj.content += templates.fileHeader.replace('{file}', element.file);
    }
    obj.content += templates.error
